...
function Get-AbrOntapLog {
    <#
    .SYNOPSIS
        Collects diagnostic information for AsBuiltReport.NetApp.ONTAP troubleshooting.
    .DESCRIPTION
    ...
    .EXAMPLE
        Get-AbrOntapLog -OutputFolderPath 'C:\Logs' -IncludeErrorDetails -PassThru
    ...
    begin {
        Write-Verbose 'Get-AbrOntapLog: Starting diagnostic collection.'
        $TimeStamp = Get-Date -Format 'yyyyMMdd_HHmmss'
        $FileName = "AbrOntapDiagnostics_$TimeStamp.json"
    ...
    process {
        ...
        try {
            ...
            Write-Host "  [Get-AbrOntapLog] Diagnostic bundle saved to: $OutputFile" -ForegroundColor Green
        } catch {
            Write-Warning "Get-AbrOntapLog  : Failed to write diagnostic file '$OutputFile': $($_.Exception.Message)"
        }
        ...
        Write-Verbose 'Get-AbrOntapLog: Diagnostic collection complete.'
    }
}
